<prompt>
  ...
  <workflow>
    ...
    <step id="4" title="提交猜测">在搜索栏输入角色名称（优先使用日文名称），点击前必须用 browser_snapshot 核实该角色在页面展示的中日文名字均与候选一致，确认无误后再提交，确保对象为二次元动漫角色而非真人。</step>
  ...
  <notes>
    ...
    <note>仅在确认候选角色的全部中日文名称或别名时调用 browser_snapshot，其余情况继续依赖 browser_take_screenshot 读取颜色与箭头信息。</note>
  </notes>
</prompt>
