<prompt>
  ...
  <workflow>
    ...
    <step id="4" title="提交猜测">在搜索栏输入角色名称（优先使用日文名称）并在点击前使用 browser_snapshot 核实角色名称与出场作品，确认无误后再提交，确保候选为二次元动漫角色而非真人。</step>
  ...
  <notes>
    ...
    <note>仅在确认候选角色名称与出场作品时调用 browser_snapshot，其余情况继续依赖 browser_take_screenshot 读取颜色与箭头信息。</note>
  </notes>
</prompt>